...
const myBot = ['ub0b6e1adca6737456b02b8950de53b89'];
...
vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
		...
		if(vx[1] == "Adminadd" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
		...
		if(txt == "Admin add" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
				vx[2] = "arg1";
		...
		}else if(txt == "Admin add" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
...
\n";
        ...
        if(txt === 'Cekik' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === 'Cekik' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
...
====================\n";
		...
		if(txt == "Tag all" && seq.toType == 2 && !isBanned(banList, seq.from_)){
			await this.tagAlls(seq);
		}else if(txt == 'Tag all' && isBanned(banList, seq.from_)){this._sendMessage(seq,"Not permitted !");}
		...
		if(txt == "Kick ban" && isAdminOrBot(seq.from_)){
			for(var i = 0; i < banList.length; i++){
		...
		}else if(txt == "Kick ban" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
       ...
       /* if(txt == 'speedtest' && isAdminOrBot(seq.from)) {
		...
		if(txt == "What time" && !isBanned(banList,seq.from_)){
...
\n# Group Cover:\nhttp://dl.profile.line.naver.jp/"+gcover;
        ...
        if(joinByUrl.includes(txt) && txt == "Gurl") {
            ...
            this._sendMessage(seq,`Line group = line://ti/g/${groupUrl}`);
        }else if(joinByUrl.includes(txt) && txt == "Gurl") {
            this._sendMessage(seq,`Updating group ...`);
            let updateGroup = await this._getGroup(seq.to);//console.info(updateGroup);
